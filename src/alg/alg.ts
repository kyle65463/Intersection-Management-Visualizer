...
function isDeadLock(vertexs: vertexWithEdge[], edges: edge[]): boolean {
    ...
    for (let i = 1; i < vertexs.length; i++) {
        if (vertexs[i - 1].id == vertexs[i].id) {
            gv.push({ id: vertexs[i].id, front: vertexs[i - 1].zone_id, end: vertexs[i].zone_id, edges: [] });
        }
    }
    for (let i = 0; i < vertexs.length; i++) {
        for (let l = 0; l < gv.length; l++) {
            if (vertexs[i].id == gv[l].id && (vertexs[i].zone_id == gv[l].end || vertexs[i].zone_id == gv[l].front)) {
                for (let j = 0; j < vertexs[i].edges.length; j++) {
                    for (let k = 0; k < gv.length; k++) {
                        if (k == l)
                            continue;
                        if (vertexs[i].edges[j].out.id == gv[k].id && (vertexs[i].edges[j].out.zone_id == gv[k].end || vertexs[i].edges[j].out.zone_id == gv[k].front)) {
                            let a = 0;
                            for (let m = 0; m < gv[l].edges.length; m++) {
                                if (gv[k] == gv[l].edges[m])
                                    a = 1;
                            }
                            if (a != 1)
                                gv[l].edges.push(gv[k]);
                        }
                    }
                }
            }
    ...
    for (let i = 0; i < edges.length; i++) {
        for (let j = 0; j < gv.length; j++) {
            if (edges[i].in.id == gv[j].id && (edges[i].in.zone_id == gv[j].front || edges[i].in.zone_id == gv[j].end)) {
                for (let k = 0; k < gv.length; k++) {
                    if (edges[i].out.id == gv[k].id && (edges[i].out.zone_id == gv[k].front || edges[i].out.zone_id == gv[k].end)) {
                        let b: number = 1;
                        for (let m = 0; m < gv[j].edges.length; m++)
                            if (gv[k] == gv[j].edges[m])
                                b = 0;
                        if (b == 1)
                            gv[j].edges.push(gv[k]);
                    }
                }
            }
        }
...
var edges: edge[] = [];
var type3edges: type3edge[] = [];
var ve: vertexWithEdge[] = [];
...
for (let i = 0; i < total_zone; i++) {
    for (let j = 0; j < zoneId_cardIdMap[i].length; j++) {
        for (let k = j + 1; k < zoneId_cardIdMap[i].length; k++) {
            if (zoneId_cardIdMap[i][j].roadDir == zoneId_cardIdMap[i][k].roadDir &&
                zoneId_cardIdMap[i][j].roadId == zoneId_cardIdMap[i][k].roadId) {
                if (zoneId_cardIdMap[i][j].idOnRoad < zoneId_cardIdMap[i][k].idOnRoad) {
                    let v = 0;
                    for (let m = 0; m < ve.length; m++)
                        if (ve[m].id == zoneId_cardIdMap[i][j].id && ve[m].zone_id == i) {
                            ve[m].edges.push({ type: 2, in: { id: zoneId_cardIdMap[i][j].id, zone_id: i }, out: { id: zoneId_cardIdMap[i][k].id, zone_id: i } });
                            break;
                        }
                    //edges.push({type:2,in:{id:zoneId_cardIdMap[i][j].id,zone_id:i},out:{id:zoneId_cardIdMap[i][k].id,zone_id:i}});
                }
            } else {
                type3edges.push({ endpoint: [{ id: zoneId_cardIdMap[i][j].id, zone_id: i }, { id: zoneId_cardIdMap[i][k].id, zone_id: i }] });
            }
        }
    }
}
...
while (true) {
    ...
    for (let i = 0; i < type3edges.length; i++) {
        ...
        removedtype3edge.push({ type: 3, in: type3edges[i].endpoint[rand], out: type3edges[i].endpoint[1 - rand] });
